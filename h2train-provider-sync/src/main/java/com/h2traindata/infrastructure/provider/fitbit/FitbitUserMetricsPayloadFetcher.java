...
class FitbitUserMetricsPayloadFetcher {
    ...
    private static final long SNAPSHOT_CACHE_TTL_SECONDS = 30L;

    private final FitbitApiClient fitbitApiClient;
    private final Executor metricsCollectorExecutor;
    private final Map<String, CachedBundle> cache = new ConcurrentHashMap<>();
    ...
    FitbitUserMetricsPayloadBundle fetch(ProviderConnection connection) {
        ...
        CachedBundle cachedBundle = cache.get(cacheKey);
        if (cachedBundle != null && !cachedBundle.expiresAt().isBefore(now)) {
            return cachedBundle.payloadBundle();
        }

        FitbitUserMetricsPayloadBundle payloadBundle = fetchFresh(connection);
        cache.put(cacheKey, new CachedBundle(payloadBundle, now.plusSeconds(SNAPSHOT_CACHE_TTL_SECONDS)));
        return payloadBundle;
    }

    private FitbitUserMetricsPayloadBundle fetchFresh(ProviderConnection connection) {
    ...
    private String cacheKey(ProviderConnection connection) {
        return connection.providerId() + ":" + connection.athlete().id() + ":" + connection.accessToken();
    }
    ...
    private record CachedBundle(FitbitUserMetricsPayloadBundle payloadBundle, Instant expiresAt) {
    }
}
