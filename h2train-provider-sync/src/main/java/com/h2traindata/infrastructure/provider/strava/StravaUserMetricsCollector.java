...
public class StravaUserMetricsCollector implements ProviderEventCollector {
    ...
    public EventBatch collect(ProviderConnection connection, SyncCursor cursor) {
        ...
        ProviderEvent event = new ProviderEvent(
                ...
                snapshotEventId(athleteId, USER_PROFILE_EVENT_NAME, snapshotAt),
                snapshotAt,
                eventFields(connection.athlete().id(), snapshotAt, athletePayload)
        );
    ...
    private Map<String, Object> eventFields(String athleteId, Instant snapshotAt, Map<String, Object> athletePayload) {
        Map<String, Object> eventFields = baseEvent(snapshotAt, providerId(), athleteId);
        put(eventFields, "weight", doubleValue(athletePayload.get("weight")));
        put(eventFields, "height", null);
        put(eventFields, "gender", normalizeGender(stringValue(athletePayload.get("sex"))));
        put(eventFields, "timezone", stringValue(athletePayload.get("timezone")));
        return eventFields;
    }
    ...
    private Map<String, Object> providerSpecificPayload(Map<String, Object> athletePayload, Map<String, Object> athleteStats) {
        ...
        put(providerSpecific, "providerAthleteId", stringValue(athletePayload.get("id")));
        put(providerSpecific, "username", stringValue(athletePayload.get("username")));
        put(providerSpecific, "displayName", firstNonBlank(fullName(athletePayload), stringValue(athletePayload.get("username"))));
        put(providerSpecific, "fullName", fullName(athletePayload));
        put(providerSpecific, "firstName", stringValue(athletePayload.get("firstname")));
        put(providerSpecific, "lastName", stringValue(athletePayload.get("lastname")));
        put(providerSpecific, "avatarUrl", stringValue(athletePayload.get("profile")));
        put(providerSpecific, "city", stringValue(athletePayload.get("city")));
        put(providerSpecific, "state", stringValue(athletePayload.get("state")));
        put(providerSpecific, "country", stringValue(athletePayload.get("country")));
        put(providerSpecific, "measurementSystem", measurementSystem(stringValue(athletePayload.get("measurement_preference"))));
        put(providerSpecific, "createdAt", instantValue(athletePayload.get("created_at")));
        put(providerSpecific, "updatedAt", instantValue(athletePayload.get("updated_at")));
        put(providerSpecific, "followerCount", intValue(athletePayload.get("follower_count")));
        put(providerSpecific, "friendCount", intValue(athletePayload.get("friend_count")));
        put(providerSpecific, "subscriptionActive", summitActive(athletePayload));
        put(providerSpecific, "ftpWatts", intValue(athletePayload.get("ftp")));
        put(providerSpecific, "lifetimeDistanceMeters", totalLifetimeDistanceMeters(athleteStats));
    ...
    private Map<String, Object> fetchAthleteStats(String accessToken, String athleteId) {
        Long athleteIdValue = com.h2traindata.infrastructure.provider.common.PayloadSupport.longValue(athleteId);
