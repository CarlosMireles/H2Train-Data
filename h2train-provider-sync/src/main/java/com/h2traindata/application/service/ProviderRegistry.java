...
@Service
public class ProviderRegistry implements ProviderCatalog {
    ...
    @Override
    public Collection<EventType> supportedEventTypes(String providerId) {
        connector(providerId);
        return collectors.keySet().stream()
                .filter(key -> key.providerId().equals(providerId))
                .map(CollectorKey::eventType)
