...
class StravaActivityCollectorTest {
    ...
    @Test
    void mapsDetailedActivitiesToWorkoutEvents() {
        ...
        assertEquals("Workout", batch.events().get(0).eventName());
        ...
        assertEquals("run", batch.events().get(0).field("activityType"));
        assertEquals(10_000.0, batch.events().get(0).field("distanceMeters"));
        assertEquals(1900, batch.events().get(0).field("duration"));
    }
}
