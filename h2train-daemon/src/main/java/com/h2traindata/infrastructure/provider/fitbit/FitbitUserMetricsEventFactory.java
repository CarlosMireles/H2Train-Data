...
import static com.h2traindata.infrastructure.provider.common.OntologyEventSupport.baseEvent;
import static com.h2traindata.infrastructure.provider.common.OntologyEventSupport.snapshotEventId;
...
class FitbitUserMetricsEventFactory {
    ...
    private static final String USER_PROFILE_EVENT_NAME = "UserProfile";
    private static final String USER_GOALS_EVENT_NAME = "UserGoals";
    private static final String ACTIVITY_SUMMARY_EVENT_NAME = "ActivitySummary";
    private static final String STEPS_EVENT_NAME = "Steps";
    private static final String DISTANCE_EVENT_NAME = "Distance";
    private static final String CALORIES_BURNED_EVENT_NAME = "CaloriesBurned";
    private static final String HEART_RATE_EVENT_NAME = "HeartRate";
    private static final String BODY_COMPOSITION_EVENT_NAME = "BodyComposition";
    private static final String NUTRITION_EVENT_NAME = "Nutrition";
    private static final String SLEEP_EVENT_NAME = "Sleep";
    private static final String BLOOD_GLUCOSE_EVENT_NAME = "BloodGlucose";
    private static final String ELECTROCARDIOGRAM_EVENT_NAME = "Electrocardiogram";
    private static final String ANOMALY_DETECTED_EVENT_NAME = "AnomalyDetected";

    List<ProviderEvent> createUserStateEvents(String providerId,
                                              String connectionAthleteId,
                                              Instant snapshotAt,
                                              FitbitUserMetricsPayloadBundle payloadBundle) {
        SnapshotData snapshot = snapshotData(snapshotAt, payloadBundle);
        List<ProviderEvent> events = new ArrayList<>();

        Map<String, Object> userProfile = baseEvent(snapshotAt, providerId, connectionAthleteId);
        put(userProfile, "weight", snapshot.normalized().get("weightKg"));
        put(userProfile, "height", snapshot.normalized().get("heightCm"));
        put(userProfile, "gender", snapshot.normalized().get("gender"));
        put(userProfile, "timezone", snapshot.normalized().get("timezone"));
        if (hasAnyValue(userProfile, "weight", "height", "gender", "timezone")) {
            events.add(snapshotEvent(
                    providerId,
                    connectionAthleteId,
                    EventType.USER_STATE,
                    USER_PROFILE_EVENT_NAME,
                    snapshotAt,
                    userProfile
            ));
        }

        addUserGoalsEvent(events, providerId, connectionAthleteId, snapshot, "daily", "Daily");
        addUserGoalsEvent(events, providerId, connectionAthleteId, snapshot, "weekly", "Weekly");
        return events;
    }

    List<ProviderEvent> createActivityEvents(String providerId,
                                             String connectionAthleteId,
                                             Instant snapshotAt,
                                             FitbitUserMetricsPayloadBundle payloadBundle) {
        SnapshotData snapshot = snapshotData(snapshotAt, payloadBundle);
        Map<String, Object> activitySummary = mapValue(snapshot.normalized().get("latestActivitySummary"));
        if (activitySummary.isEmpty()) {
            return List.of();
        }

        Instant recordedAt = instantValue(activitySummary.get("recordedAt"));
        Map<String, Object> payload = baseEvent(recordedAt != null ? recordedAt : snapshotAt, providerId, connectionAthleteId);
        put(payload, "steps", activitySummary.get("steps"));
        put(payload, "distanceMeters", activitySummary.get("distanceMeters"));
        put(payload, "caloriesOut", activitySummary.get("caloriesOutKcal"));
        put(payload, "activityCalories", activitySummary.get("activityCaloriesKcal"));
        put(payload, "sedentaryMinutes", activitySummary.get("sedentaryMinutes"));
        put(payload, "lightActivityMinutes", activitySummary.get("lightlyActiveMinutes"));
        put(payload, "moderateActivityMinutes", activitySummary.get("fairlyActiveMinutes"));
        put(payload, "intenseActivityMinutes", activitySummary.get("veryActiveMinutes"));

        return List.of(snapshotEvent(
                providerId,
                connectionAthleteId,
                EventType.ACTIVITY,
                ACTIVITY_SUMMARY_EVENT_NAME,
                payload.get("timestamp") instanceof Instant timestamp ? timestamp : snapshotAt,
                payload
        ));
    }

    List<ProviderEvent> createPhysiologicalEvents(String providerId,
                                                  String connectionAthleteId,
                                                  Instant snapshotAt,
                                                  FitbitUserMetricsPayloadBundle payloadBundle) {
        SnapshotData snapshot = snapshotData(snapshotAt, payloadBundle);
        ...
        Map<String, Object> activitySummary = mapValue(snapshot.normalized().get("latestActivitySummary"));
        Instant recordedAt = instantValue(activitySummary.get("recordedAt"));
        Instant eventTs = recordedAt != null ? recordedAt : snapshotAt;

        Map<String, Object> steps = baseEvent(eventTs, providerId, connectionAthleteId);
        put(steps, "steps", activitySummary.get("steps"));
        addIfPresent(events, snapshotEvent(providerId, connectionAthleteId, EventType.PHYSIOLOGICAL, STEPS_EVENT_NAME,
                eventTs, steps), "steps");

        Map<String, Object> distance = baseEvent(eventTs, providerId, connectionAthleteId);
        put(distance, "distanceMeters", activitySummary.get("distanceMeters"));
        addIfPresent(events, snapshotEvent(providerId, connectionAthleteId, EventType.PHYSIOLOGICAL, DISTANCE_EVENT_NAME,
                eventTs, distance), "distanceMeters");

        Map<String, Object> caloriesBurned = baseEvent(eventTs, providerId, connectionAthleteId);
        put(caloriesBurned, "calories", activitySummary.get("caloriesOutKcal"));
        addIfPresent(events, snapshotEvent(providerId, connectionAthleteId, EventType.PHYSIOLOGICAL, CALORIES_BURNED_EVENT_NAME,
                eventTs, caloriesBurned), "calories");

        Map<String, Object> heartRateSummary = mapValue(snapshot.normalized().get("latestHeartRateSummary"));
        List<Map<String, Object>> zones = listOfMaps(heartRateSummary.get("heartRateZones"));
        for (Map<String, Object> zone : zones) {
            Map<String, Object> heartRate = baseEvent(eventTs, providerId, connectionAthleteId);
            put(heartRate, "zone", firstNonBlank(stringValue(zone.get("name")), stringValue(zone.get("type"))));
            put(heartRate, "min", intValue(zone.get("min")));
            put(heartRate, "max", intValue(zone.get("max")));
            put(heartRate, "minutes", intValue(zone.get("minutes")));
            put(heartRate, "calories", doubleValue(firstNonNull(zone.get("caloriesOut"), zone.get("calories"))));
            addIfPresent(events, snapshotEvent(
                    providerId,
                    connectionAthleteId,
                    EventType.PHYSIOLOGICAL,
                    HEART_RATE_EVENT_NAME,
                    eventTs,
                    stringValue(heartRate.get("zone")),
                    heartRate
            ), "zone", "minutes", "min", "max", "calories");
        }
        return events;
    }

    List<ProviderEvent> createBodyCompositionEvents(String providerId,
                                                    String connectionAthleteId,
                                                    Instant snapshotAt,
                                                    FitbitUserMetricsPayloadBundle payloadBundle) {
        SnapshotData snapshot = snapshotData(snapshotAt, payloadBundle);
        ...
        Map<String, Object> bodyComposition = mapValue(snapshot.normalized().get("latestBodyComposition"));
        Instant bodyTs = instantValue(bodyComposition.get("recordedAt"));
        Map<String, Object> bodyPayload = baseEvent(bodyTs != null ? bodyTs : snapshotAt, providerId, connectionAthleteId);
        put(bodyPayload, "weight", bodyComposition.get("weightKg"));
        put(bodyPayload, "bodyMassIndex", bodyComposition.get("bmi"));
        put(bodyPayload, "bodyFatPercentage", bodyComposition.get("bodyFatPct"));
        addIfPresent(events, snapshotEvent(
                providerId,
                connectionAthleteId,
                EventType.BODY_COMPOSITION,
                BODY_COMPOSITION_EVENT_NAME,
                bodyPayload.get("timestamp") instanceof Instant timestamp ? timestamp : snapshotAt,
                bodyPayload
        ), "weight", "bodyMassIndex", "bodyFatPercentage");

        Map<String, Object> nutrition = mapValue(snapshot.normalized().get("latestNutrition"));
        Instant nutritionTs = instantValue(nutrition.get("recordedAt"));
        Map<String, Object> nutritionPayload = baseEvent(nutritionTs != null ? nutritionTs : snapshotAt, providerId, connectionAthleteId);
        put(nutritionPayload, "calories", nutrition.get("caloriesInKcal"));
        put(nutritionPayload, "water", nutrition.get("water"));
        addIfPresent(events, snapshotEvent(
                providerId,
                connectionAthleteId,
                EventType.BODY_COMPOSITION,
                NUTRITION_EVENT_NAME,
                nutritionPayload.get("timestamp") instanceof Instant timestamp ? timestamp : snapshotAt,
                nutritionPayload
        ), "calories", "water");
    ...
    List<ProviderEvent> createHealthEvents(String providerId,
                                           String connectionAthleteId,
                                           Instant snapshotAt,
                                           FitbitUserMetricsPayloadBundle payloadBundle) {
        SnapshotData snapshot = snapshotData(snapshotAt, payloadBundle);
        ...
        Map<String, Object> sleep = mapValue(snapshot.normalized().get("latestSleep"));
        Instant sleepTs = firstInstant(instantValue(sleep.get("startTime")), instantValue(sleep.get("endTime")), snapshotAt);
        Map<String, Object> sleepPayload = baseEvent(sleepTs, providerId, connectionAthleteId);
        put(sleepPayload, "startTime", sleep.get("startTime"));
        put(sleepPayload, "endTime", sleep.get("endTime"));
        put(sleepPayload, "duration", sleep.get("durationSeconds"));
        addIfPresent(events, snapshotEvent(
                providerId,
                connectionAthleteId,
                EventType.HEALTH,
        ...
        Map<String, Object> bloodGlucose = mapValue(snapshot.normalized().get("latestBloodGlucose"));
        Instant bloodGlucoseTs = instantValue(bloodGlucose.get("recordedAt"));
        Map<String, Object> bloodGlucosePayload = baseEvent(
                bloodGlucoseTs != null ? bloodGlucoseTs : snapshotAt,
                providerId,
                connectionAthleteId
        );
        put(bloodGlucosePayload, "averageGlucose", bloodGlucose.get("value"));
        addIfPresent(events, snapshotEvent(
                providerId,
                connectionAthleteId,
                EventType.HEALTH,
                BLOOD_GLUCOSE_EVENT_NAME,
                bloodGlucosePayload.get("timestamp") instanceof Instant timestamp ? timestamp : snapshotAt,
                bloodGlucosePayload
        ), "averageGlucose");

        Map<String, Object> ecg = mapValue(snapshot.normalized().get("latestElectrocardiogram"));
        Instant ecgTs = instantValue(ecg.get("recordedAt"));
        Map<String, Object> ecgPayload = baseEvent(ecgTs != null ? ecgTs : snapshotAt, providerId, connectionAthleteId);
        put(ecgPayload, "measurementTimestamp", ecg.get("recordedAt"));
        put(ecgPayload, "electrocardiogramData", electrocardiogramData(ecg));
        addIfPresent(events, snapshotEvent(
                providerId,
                connectionAthleteId,
                EventType.HEALTH,
                ELECTROCARDIOGRAM_EVENT_NAME,
                ecgPayload.get("timestamp") instanceof Instant timestamp ? timestamp : snapshotAt,
                ecgPayload
        ), "measurementTimestamp", "electrocardiogramData");

        Map<String, Object> anomaly = mapValue(snapshot.normalized().get("latestIrregularRhythmNotification"));
        Instant anomalyTs = firstInstant(instantValue(anomaly.get("detectedAt")), instantValue(anomaly.get("alertedAt")), snapshotAt);
        Map<String, Object> anomalyPayload = baseEvent(anomalyTs, providerId, connectionAthleteId);
        put(anomalyPayload, "detectedTimestamp", anomalyTs);
        put(anomalyPayload, "metric", "irregularRhythmNotification");
        put(anomalyPayload, "description", anomalyDescription(anomaly));
        addIfPresent(events, snapshotEvent(
                providerId,
                connectionAthleteId,
                EventType.HEALTH,
                ANOMALY_DETECTED_EVENT_NAME,
                anomalyTs,
                anomalyPayload
        ), "description");
    ...
    private void addUserGoalsEvent(List<ProviderEvent> events,
                                   String providerId,
                                   String connectionAthleteId,
                                   SnapshotData snapshot,
                                   String qualifier,
                                   String scale) {
        Map<String, Object> goals = mapValue(snapshot.normalized().get("activityGoals" + capitalize(qualifier)));
        if (goals.isEmpty()) {
            return;
        }
        Map<String, Object> payload = baseEvent(snapshot.snapshotAt(), providerId, connectionAthleteId);
        put(payload, "scale", scale);
        put(payload, "steps", goals.get("steps"));
        put(payload, "calories", goals.get("caloriesOutKcal"));
        put(payload, "distance", goals.get("distanceMeters"));
        put(payload, "activeMinutes", goals.get("activeMinutes"));
        if (hasAnyValue(payload, "steps", "calories", "distance", "activeMinutes")) {
            events.add(snapshotEvent(
                    providerId,
                    connectionAthleteId,
                    EventType.USER_STATE,
                    USER_GOALS_EVENT_NAME,
                    snapshot.snapshotAt(),
                    qualifier,
                    payload
            ));
    ...
    private ProviderEvent snapshotEvent(String providerId,
                                        String connectionAthleteId,
                                        EventType eventType,
                                        String eventName,
                                        Instant occurredAt,
                                        Map<String, Object> eventFields) {
        return snapshotEvent(providerId, connectionAthleteId, eventType, eventName, occurredAt, null, eventFields);
    }
    ...
    private ProviderEvent snapshotEvent(String providerId,
        ...
        return new ProviderEvent(
                ...
                eventType,
                eventName,
                snapshotEventId(connectionAthleteId, eventName, qualifier, timestamp),
                timestamp,
                eventFields
        );
    }

    private void addIfPresent(List<ProviderEvent> events, ProviderEvent event, String... keys) {
        if (hasAnyValue(event.fields(), keys)) {
            events.add(event);
        }
    }

    private boolean hasAnyValue(Map<String, Object> payload, String... keys) {
        for (String key : keys) {
    ...
    private Map<String, Object> subset(Map<String, Object> source, String... keys) {
    ...
    private Map<String, Object> electrocardiogramData(Map<String, Object> electrocardiogram) {
        Map<String, Object> values = new LinkedHashMap<>();
        put(values, "durationSeconds", electrocardiogram.get("durationSeconds"));
        put(values, "resultClassification", electrocardiogram.get("resultClassification"));
        put(values, "heartRateBeatsPerMinute", electrocardiogram.get("heartRateBpm"));
        put(values, "symptoms", electrocardiogram.get("symptoms"));
        put(values, "deviceName", electrocardiogram.get("deviceName"));
        return values;
    }

    private SnapshotData snapshotData(Instant snapshotAt, FitbitUserMetricsPayloadBundle payloadBundle) {
    ...
    private String anomalyDescription(Map<String, Object> anomaly) {
        String result = stringValue(anomaly.get("result"));
        String status = stringValue(anomaly.get("status"));
        if (!StringUtils.hasText(result) && !StringUtils.hasText(status)) {
            return null;
        }
    ...
    private Map<String, Object> normalizedSnapshot(Map<String, Object> user,
